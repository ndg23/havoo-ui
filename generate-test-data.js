...
async function generateTestData() {
    ...
    if (professionsError) {
        console.error('Erreur lors de la création des catégories:', professionsError)
        return
    }
    ...
    for (let i = 0; i < NUM_SERVICES; i++) {
        services.push({
            profession_id: professionsData[Math.floor(Math.random() * professionsData.length)].id,
    ...
    if (missionsError) {
        console.error('Erreur lors de la création des demandes:', missionsError)
        return
    }
    ...
    for (let i = 0; i < NUM_PROPOSALS; i++) {
        ...
        proposals.push({
            ...
            price: mission.budget * (0.8 + Math.random() * 0.4), // +/- 20% du budget
    ...
    const uniqueProposals = proposals.filter((proposal, index, self) =>
        index === self.findIndex(p =>
            p.mission_id === proposal.mission_id && p.expert_id === proposal.expert_id
        )
    )
