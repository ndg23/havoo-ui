<template>
    <div class="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <!-- Tableau des demandes avec UTable -->
      <div v-else>
        <UTable
          :columns="columns"
        ...
        >
          ...
          <template #client-data="{ row }">
            <div>
              <p class="text-sm font-medium text-gray-900 dark:text-white">{{ row.client_name }}</p>
              <div class="flex items-center text-xs text-gray-500 dark:text-gray-400 mt-1">
                <MapPin class="h-3 w-3 mr-1" /> 
                {{ row.location || 'Non spécifié' }}
              </div>
            </div>
          </template>
          ...
          <template #budget-data="{ row }">
            <span class="inline-flex items-center px-3 py-1.5 bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-400 rounded-full text-xs font-bold">
              {{ row.budget ? `${row.budget}FCFA` : 'Non défini' }}
            </span>
          </template>
          ...
          <template #actions-data="{ row }">
            <div class="flex justify-end gap-2">
              ...
              <button 
                v-if="['open', 'assigned'].includes(row.status)"
          ...
          <div v-if="showAssignModal" class="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center p-4 z-50">
            ...
            >
              <div 
                ...
                class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-auto"
                @click.stop
              >
                ...
                <div class="p-6 space-y-6">
                  ...
                  <div class="space-y-4">
                    <div>
                      <label for="expert" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    ...
                    <div>
                      ...
                      <textarea 
                        ...
                        rows="3"
                        class="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:text-white"
                        placeholder="Ajouter des notes pour l'expert..."
                      ></textarea>
                    </div>
                  </div>
                </div>
                ...
                <div class="p-6 border-t border-gray-100 dark:border-gray-700 flex justify-end gap-3">
                  <button 
                    @click="showAssignModal = false"
                    class="px-4 py-2 border border-gray-200 dark:border-gray-600 rounded-xl text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
                  >
                    Annuler
                  </button>
                  <button 
                    @click="assignExpert"
                    class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-xl flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    :disabled="!selectedExpertId || isAssigning"
                  >
                    <Loader2 v-if="isAssigning" class="h-4 w-4 animate-spin" />
                    <span>{{ isAssigning ? 'Assignation...' : 'Assigner' }}</span>
                  </button>
                </div>
              </div>
            </Transition>
  ...
  const { 
    RefreshCw, Plus, Search, Eye, Edit, Trash2, X, Info, CheckCircle, 
    AlertTriangle, FileText, UserPlus, Loader2, Download, MapPin, 
